...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'expirationDate' | 'createdAt'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...product,
      expirationDate: product.expirationDate?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...restProduct,
      expirationDate: restProduct.expirationDate ? dayjs(restProduct.expirationDate) : undefined,
